...
class ConvModel(object):
    def __init__(self, drop_out=False, is_training=True):
      ...
      self.h_conv1 = conv2d(self.x_image, binarize(self.W_conv1), 2) + self.b_conv1
      ...
      self.h_conv2 = conv2d(binarize(self.out_feature1), binarize(self.W_conv2), 2) + self.b_conv2
      ...
      self.h_conv3 = conv2d(binarize(self.out_feature2), binarize(self.W_conv3), 2) + self.b_conv3
      ...
      self.h_conv4 = conv2d(binarize(self.out_feature3), binarize(self.W_conv4), 1) + self.b_conv4
      ...
      self.h_conv5 = conv2d(binarize(self.out_feature4), binarize(self.W_conv5), 1) + self.b_conv5
      ...
      self.out_feature5_flat = tf.reshape(binarize(self.out_feature5), [-1, 1152])
      self.h_fc1 = tf.matmul(self.out_feature5_flat, binarize(self.W_fc1)) + self.b_fc1
      ...
      if drop_out:
          self.out_feature6 = tf.nn.dropout(self.out_feature6, self.keep_prob)

      # FCL 2
      self.W_fc2 = weight_variable([1164, 100])
      self.b_fc2 = bias_variable([100])

      self.h_fc2 = tf.matmul(binarize(self.out_feature6), binarize(self.W_fc2)) + self.b_fc2
      ...
      if drop_out:
          self.out_feature7 = tf.nn.dropout(self.out_feature7, self.keep_prob)

      # FCL 3
      self.W_fc3 = weight_variable([100, 50])
      self.b_fc3 = bias_variable([50])

      self.h_fc3 = tf.matmul(binarize(self.out_feature7), binarize(self.W_fc3)) + self.b_fc3
      ...
      if drop_out:
          self.out_feature8 = tf.nn.dropout(self.out_feature8, self.keep_prob)

      # FCL 3
      self.W_fc4 = weight_variable([50, 10])
      self.b_fc4 = bias_variable([10])

      self.h_fc4 = tf.matmul(binarize(self.out_feature8), binarize(self.W_fc4)) + self.b_fc4
      ...
      if drop_out:
          self.out_feature9 = tf.nn.dropout(self.out_feature9, self.keep_prob)
      ...
      self.y = tf.multiply(tf.atan(tf.matmul(self.out_feature9, self.W_fc5) + self.b_fc5), 2)
